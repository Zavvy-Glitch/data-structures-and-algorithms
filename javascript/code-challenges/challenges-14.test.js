...
const toTitleCase = (arr) => {
  // Solution code here...
  let newArr = arr.map(value => value[0].toUpperCase() + value.substring(1));
  return newArr;
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let filteredPeople = arr.filter(function (currentElement) {
    return currentElement.mass > 77 ;
  });
  let newArr = filteredPeople.map(person => {
    return person.name;
  });
  let names = newArr.join(' - ');
  return names;
};
